...
} from "@/lib/audit/lara-markets-delivery-collector";
...
export async function POST(request: NextRequest) {
  ...
  if (
    ...
    return response({ error: "Invalid markets-delivery-map request." }, 400);
  }

  try {
    // Connection, shop, queries and durable run ID are all server-owned. The
    // caller can only trigger this one fixed, read-only configuration map.
    ...
    const result = await runLaraMarketsDeliveryCollector({ requestedBy });
    ...
    console.error("The fixed Lara Markets and delivery map request failed.");
    return response(
      {
        error: "The Markets and delivery map could not be completed.",
