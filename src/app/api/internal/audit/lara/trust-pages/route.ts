...
} from "@/lib/audit/lara-trust-pages-repair";
import { LARA_AUDIT_CONNECTION } from "@/lib/audit/shopify-lara";

export const dynamic = "force-dynamic";

const MAX_BODY_BYTES = 1_024;
const APPLY_CONFIRMATION = "apply-lara-contact-about-approved-copy";
...
export async function POST(request: NextRequest) {
  ...
  if (!body || typeof body !== "object" || Array.isArray(body)) {
    return response({ error: "Invalid trust-pages request." }, 400);
  }
  ...
  try {
    if (requestBody.action === "dry-run") {
      ...
      const dryRun = await buildLaraTrustPagesDryRun();
      return response({ ok: true, ...dryRun });
    }

    if (requestBody.action !== "apply") {
      return response({ error: "Invalid trust-pages action." }, 400);
    }
    if (
    ...
    // Shop, connection, page ids, HTML bodies, plan digest and the one-shot run
    // id all remain server-owned. The entrypoint creates the current dry run
    // and binds apply to it, or resumes its already-durable manifest.
    ...
    const result = await runLaraTrustPagesRepairOneShot({ requestedBy });
    ...
    if (error instanceof LaraTrustPagesRepairError) {
      return response({ error: error.code }, 409);
    }
    console.error("The exact Lara trust-pages request failed.");
    return response({ error: "The trust-pages request could not be completed." }, 500);
  }
}
