...
comment on function public.create_manual_referral_invoice(
  ...
  'Creates the exact v3 weekly settlement: per-store agency fee lines validated
against Google boundary evidence, plus one rev_share line per account that
opted into collection revenue-share tracking, validated against the
daily_metrics rollup. Amount must equal all lines together.';
